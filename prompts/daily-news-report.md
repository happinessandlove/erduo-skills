...
按照下方"输出模板"生成 Markdown 文件，写入 `NewsReport/YYYY-MM-DD-news-report.md`。
...
```yaml
source_id: string      # 来源标识（如 hn, hf_papers）
title: string          # 标题（中文标题优先，技术术语保留英文）
...
- **生成时间**: YYYY-MM-DD (北京时间)
- **使用源层级**: Tier 1 / Tier 1+2 / Tier 1+2+3
- **质量阈值**: 最低 X 分收录
- **去重策略**: URL 匹配 + 标题相似度 + 历史缓存检查
- **评分分布**:
  - ⭐⭐⭐⭐⭐ (5分): X 条 (Y%)
  - ⭐⭐⭐⭐ (4分): X 条 (Y%)
  - ⭐⭐⭐ (3分): X 条 (Y%)
